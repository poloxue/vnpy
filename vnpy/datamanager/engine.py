...
class ManagerEngine(BaseEngine):
    ...
    def delete_bar_data(
        ...
        """"""
        count: int = self.database.delete_bar_data(symbol, exchange, interval)
    ...
    def download_tick_data(
        self, symbol: str, exchange: Exchange, start: datetime, output: Callable
    ) -> int:
        """
        Query tick data from datafeed.
        """
        req: HistoryRequest = HistoryRequest(
            symbol=symbol, exchange=exchange, start=start, end=datetime.now(DB_TZ)
        )
