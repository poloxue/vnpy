...
class BinanceInverseGateway(BaseGateway):
    ...
    def __init__(self, event_engine: EventEngine, gateway_name: str) -> None:
        ...
        self.trade_ws_api: "BinanceInverseTradeWebsocketApi" = (
            BinanceInverseTradeWebsocketApi(self)
        )
        self.market_ws_api: "BinanceInverseDataWebsocketApi" = (
            BinanceInverseDataWebsocketApi(self)
        )
    ...
    def query_history(self, req: HistoryRequest) -> List[BarData]:
        ...
        while True:
            ...
            if resp.status_code // 100 != 2:
                msg: str = (
                    f"获取历史数据失败，状态码：{resp.status_code}，信息：{resp.text}"
                )
                self.gateway.write_log(msg)
                break
            else:
