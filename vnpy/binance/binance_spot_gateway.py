...
from vnpy.rest import RestClient, Request, Response
...
class BinanceSpotRestAPi(RestClient):
    ...
    def connect(
        self, key: str, secret: str, proxy_host: str, proxy_port: int, server: str
    ) -> None:
        """连接REST服务器"""
        self.key = key
...
class BinanceSpotDataWebsocketApi(WebsocketClient):
    ...
    def subscribe(self, req: SubscribeRequest) -> None:
        ...
        channels = [f"{req.symbol}@ticker", f"{req.symbol}@depth5"]
