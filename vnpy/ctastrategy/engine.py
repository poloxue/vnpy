...
class CtaEngine(BaseEngine):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine) -> None:
        ...
        self.strategy_logs: Dict[str, deque[LogData]] = defaultdict(
    ...
    def cancel_server_orders(self, strategy: CtaTemplate, vt_orderids: set) -> None:
        """
        ...
        self.main_engine.cancel_orders(requests, order.gateway_name)
    ...
    def cancel_orders(self, strategy: CtaTemplate, vt_orderids: set) -> None:
        """ """
        contract: ContractData = self.main_engine.get_contract(strategy.vt_symbol)
        if not contract:
            return

        batch_cancel = contract.batch_cancel
        remain_vt_orderids = set()
        for vt_orderid in vt_orderids:
            if vt_orderid.startswith(STOPORDER_PREFIX):
                self.cancel_local_stop_order(strategy, vt_orderid)
            else:
                if not batch_cancel:
                    ...
                    remain_vt_orderids.add(vt_orderid)

        if not len(remain_vt_orderids):
            return

        self.cancel_server_orders(strategy, remain_vt_orderids)
    ...
    def cancel_all(self, strategy: CtaTemplate) -> None:
        ...
        self.cancel_orders(strategy, vt_orderids)
    ...
    def load_bar(
        ...
        if not use_database:
            ...
            if contract and contract.history_data:
                ...
                bars: List[BarData] = self.main_engine.query_history(
                    req, contract.gateway_name
                )

            # Try to query bars from datafeed, if not found, load from database.
            else:
                bars: List[BarData] = self.query_bar_from_datafeed(
    ...
    def _init_strategy(self, strategy_name: str) -> None:
        ...
        if strategy.inited:
            self.write_log(f"已经完成初始化，禁止重复操作", strategy)
            return

        self.write_log(f"开始执行初始化", strategy)
        ...
        self.put_strategy_event(strategy)
        self.write_log(f"初始化完成", strategy)
    ...
    def remove_strategy(self, strategy_name: str) -> bool:
        ...
        strategy: CtaTemplate = self.strategies[strategy_name]
        if strategy.trading:
            self.write_log(f"策略{strategy.strategy_name}移除失败，请先停止", strategy)
            return
    ...
    def load_strategy_class(self) -> None:
        ...
        """
        path: Path = Path.cwd().joinpath("strategies/cta")
    ...
    def write_log(self, msg: str, strategy: CtaTemplate = None) -> None:
        ...
        if strategy:
            ...
            extras["strategy_name"] = strategy.strategy_name

        log: LogData = LogData(msg=msg, gateway_name=APP_NAME, extras=extras)
        if strategy:
            self.strategy_logs[strategy.strategy_name].append(log)
    ...
    def get_strategy_logs(self, strategy_name) -> List:
        return [
            {
            ...
            for log in self.strategy_logs[strategy_name]
        ]
